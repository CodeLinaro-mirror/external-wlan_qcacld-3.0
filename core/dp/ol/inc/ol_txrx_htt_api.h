...
/**
 * @brief Process MLO peer map message sent by the target
 * @details
 *  Each time the target allocates a new mlo peer ID, it will inform the
 *  host via the "mlo peer map" message.  This function processes that
 *  message.  The host data SW looks for a mld peer object whose MAC address
 *  matches the MAC address specified in the mlo peer map message, and then
 *  sets up a mapping between the peer ID specified in the message and
 *  the peer object that was found.
 *
 * @param pdev - data physical device handle
...
QDF_STATUS ol_rx_mlo_peer_map_handler(struct ol_txrx_pdev_t *pdev,
				      ...
				      uint8_t vdev_id);
/**
 * @brief Process a mlo peer unmap message sent by the target.
 * @details
 *  Each time the target frees a mlo peer ID, it will inform the host via the
 *  "mlo peer unmap" message.  This function processes that message.
 *  The host data SW uses the peer ID from the message to find the peer
 *  object from peer_map[peer_id], then invalidates peer_map[peer_id]
 *  (by setting it to NULL), and checks whether there are any remaining
 *  references to the peer object.  If not, the function deletes the
