 ...
 * Thu Nov 16 11:09:21 2023 from the following file(s):
 *
 * dot11f.frms
 *
...
#define SigIeExtRequestedInfo (0x000a)
...
#define SigIebeacon_report_frm_body_fragment_id (0x0021)
					     ...
					     bool append_ie)
{
	...
	if (!ielen) {
		...
		switch (pDst->max_tx_pwr_interpret) {
		case 0:
			if (unlikely(ielen < 1)) {
				pDst->present = 0;
				return DOT11F_INCOMPLETE_IE;
			...
			pDst->ext_max_tx_power.ext_max_tx_power_local_eirp.max_tx_power_for_320 = *pBuf;
			pBuf += 1;
			ielen -= (uint8_t)1;
			break;
		case 1:
			...
			pBuf += 1;
			ielen -= 1;
			pDst->ext_max_tx_power.ext_max_tx_power_local_psd.ext_count = tmp33__ >> 0 & 0xf;
			pDst->ext_max_tx_power.ext_max_tx_power_local_psd.reserved = tmp33__ >> 4 & 0xf;
			if (unlikely(ielen < 8)) {
				pDst->present = 0;
				return DOT11F_INCOMPLETE_IE;
			}

			DOT11F_MEMCPY(pCtx, pDst->ext_max_tx_power.ext_max_tx_power_local_psd.max_tx_psd_power, pBuf, 8);
			pBuf += 8;
			ielen -= (uint8_t)8;
			break;
		case 2:
			...
			pDst->ext_max_tx_power.ext_max_tx_power_reg_eirp.max_tx_power_for_320 = *pBuf;
			pBuf += 1;
			ielen -= (uint8_t)1;
			break;
		case 3:
			...
			pBuf += 1;
			ielen -= 1;
			pDst->ext_max_tx_power.ext_max_tx_power_reg_psd.ext_count = tmp34__ >> 0 & 0xf;
			pDst->ext_max_tx_power.ext_max_tx_power_reg_psd.reserved = tmp34__ >> 4 & 0xf;
			if (unlikely(ielen < 8)) {
				pDst->present = 0;
				return DOT11F_INCOMPLETE_IE;
			}

			DOT11F_MEMCPY(pCtx, pDst->ext_max_tx_power.ext_max_tx_power_reg_psd.max_tx_psd_power, pBuf, 8);
			pBuf += 8;
			ielen -= (uint8_t)8;
			break;
		}
...
static const tIEDefn IES_ChannelSwitchWrapper[] = {
	...
	0, 4, 20, SigIetransmit_power_env, {0, 0, 0, 0, 0},
				       ...
				       bool append_ie)
{
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbe_aifsn = tmp35__ >> 0 & 0xf;
	pDst->acbe_acm = tmp35__ >> 4 & 0x1;
	pDst->acbe_aci = tmp35__ >> 5 & 0x3;
	pDst->unused1 = tmp35__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbe_acwmin = tmp36__ >> 0 & 0xf;
	pDst->acbe_acwmax = tmp36__ >> 4 & 0xf;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbk_aifsn = tmp37__ >> 0 & 0xf;
	pDst->acbk_acm = tmp37__ >> 4 & 0x1;
	pDst->acbk_aci = tmp37__ >> 5 & 0x3;
	pDst->unused2 = tmp37__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbk_acwmin = tmp38__ >> 0 & 0xf;
	pDst->acbk_acwmax = tmp38__ >> 4 & 0xf;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvi_aifsn = tmp39__ >> 0 & 0xf;
	pDst->acvi_acm = tmp39__ >> 4 & 0x1;
	pDst->acvi_aci = tmp39__ >> 5 & 0x3;
	pDst->unused3 = tmp39__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvi_acwmin = tmp40__ >> 0 & 0xf;
	pDst->acvi_acwmax = tmp40__ >> 4 & 0xf;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvo_aifsn = tmp41__ >> 0 & 0xf;
	pDst->acvo_acm = tmp41__ >> 4 & 0x1;
	pDst->acvo_aci = tmp41__ >> 5 & 0x3;
	pDst->unused4 = tmp41__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvo_acwmin = tmp42__ >> 0 & 0xf;
	pDst->acvo_acwmax = tmp42__ >> 4 & 0xf;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
				  ...
				  bool append_ie)
{
	...
	tmp43__ = *pBuf;
	pDst->non_erp_present = tmp43__ >> 0 & 0x1;
	pDst->use_prot = tmp43__ >> 1 & 0x1;
	pDst->barker_preamble = tmp43__ >> 2 & 0x1;
					 ...
					 bool append_ie)
{
	...
	tmp44__ = *pBuf;
	pDst->mbssid_mask = tmp44__ >> 0 & 0x7;
	pDst->reserved = tmp44__ >> 3 & 0x1f;
	(void)pCtx;
	return status;
				 ...
				 bool append_ie)
{
	...
	pDst->chan_center_freq_seg2 = tmp52__ >> 5 & 0xff;
	pDst->reserved = tmp52__ >> 13 & 0x7;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
					 ...
					 bool append_ie)
{
	...
	pDst->vht_160_80p80_supp = tmp61__ >> 2 & 0x1;
	pDst->no_ldpc = tmp61__ >> 3 & 0x1;
					     ...
					     bool append_ie)
{
	...
	tmp77__ = *pBuf;
	pDst->no_fragmentation = tmp77__ >> 0 & 0x1;
	pDst->he_frag_operation = tmp77__ >> 1 & 0x3;
	pDst->reserved = tmp77__ >> 3 & 0x3;
	pDst->extd_buff_size = tmp77__ >> 5 & 0x7;
	(void)pCtx;
	return status;
					   ...
					   bool append_ie)
{
	...
	tmp78__ = *pBuf;
	pDst->new_color = tmp78__ >> 0 & 0x3f;
	pDst->reserved = tmp78__ >> 6 & 0x3;
	(void)pCtx;
	return status;
					      ...
					      bool append_ie)
{
	...
	tmp79__ = *pBuf;
	pDst->prot_keep_alive_reqd = tmp79__ >> 0 & 0x1;
	pDst->reserved = tmp79__ >> 1 & 0x7f;
	(void)pCtx;
	return status;
				  ...
				  bool append_ie)
{
	...
	pBuf += 2;
	ielen -= 2;
	pDst->epcs_pri_access = tmp80__ >> 0 & 0x1;
	pDst->eht_om_ctl = tmp80__ >> 1 & 0x1;
	pDst->triggered_txop_sharing_mode1 = tmp80__ >> 2 & 0x1;
	pDst->triggered_txop_sharing_mode2 = tmp80__ >> 3 & 0x1;
	pDst->restricted_twt = tmp80__ >> 4 & 0x1;
	pDst->scs_traffic_desc = tmp80__ >> 5 & 0x1;
	pDst->max_mpdu_len = tmp80__ >> 6 & 0x3;
	pDst->max_a_mpdu_len_exponent_ext = tmp80__ >> 8 & 0x1;
	pDst->eht_trs_support = tmp80__ >> 9 & 0x1;
	pDst->txop_return_support_txop_share_m2 = tmp80__ >> 10 & 0x1;
	pDst->two_bqrs_support = tmp80__ >> 11 & 0x1;
	pDst->eht_link_adaptation_support = tmp80__ >> 12 & 0x3;
	pDst->reserved = tmp80__ >> 14 & 0x3;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pDst->support_320mhz_6ghz = tmp81__ >> 1 & 0x1;
	pDst->ru_242tone_wt_20mhz = tmp81__ >> 2 & 0x1;
	pDst->ndp_4x_eht_ltf_3dot2_us_gi = tmp81__ >> 3 & 0x1;
	pDst->partial_bw_mu_mimo = tmp81__ >> 4 & 0x1;
	pDst->su_beamformer = tmp81__ >> 5 & 0x1;
	pDst->su_beamformee = tmp81__ >> 6 & 0x1;
	pDst->bfee_ss_le_80mhz = tmp81__ >> 7 & 0x7;
	pDst->bfee_ss_160mhz = tmp81__ >> 10 & 0x7;
	pDst->bfee_ss_320mhz = tmp81__ >> 13 & 0x7;
	pDst->num_sounding_dim_le_80mhz = tmp81__ >> 16 & 0x7;
	pDst->num_sounding_dim_160mhz = tmp81__ >> 19 & 0x7;
	pDst->num_sounding_dim_320mhz = tmp81__ >> 22 & 0x7;
	pDst->ng_16_su_feedback = tmp81__ >> 25 & 0x1;
	pDst->ng_16_mu_feedback = tmp81__ >> 26 & 0x1;
	pDst->cb_sz_4_2_su_feedback = tmp81__ >> 27 & 0x1;
	pDst->cb_sz_7_5_su_feedback = tmp81__ >> 28 & 0x1;
	pDst->trig_su_bforming_feedback = tmp81__ >> 29 & 0x1;
	pDst->trig_mu_bforming_partial_bw_feedback = tmp81__ >> 30 & 0x1;
	pDst->triggered_cqi_feedback = tmp81__ >> 31 & 0x1;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->partial_bw_dl_mu_mimo = tmp82__ >> 0 & 0x1;
	pDst->psr_based_sr = tmp82__ >> 1 & 0x1;
	pDst->power_boost_factor = tmp82__ >> 2 & 0x1;
	pDst->eht_mu_ppdu_4x_ltf_0_8_us_gi = tmp82__ >> 3 & 0x1;
	pDst->max_nc = tmp82__ >> 4 & 0xf;
	pDst->non_trig_cqi_feedback = tmp82__ >> 8 & 0x1;
	pDst->tx_1024_4096_qam_lt_242_tone_ru = tmp82__ >> 9 & 0x1;
	pDst->rx_1024_4096_qam_lt_242_tone_ru = tmp82__ >> 10 & 0x1;
	pDst->ppet_present = tmp82__ >> 11 & 0x1;
	pDst->common_nominal_pkt_padding = tmp82__ >> 12 & 0x3;
	pDst->max_num_eht_ltf = tmp82__ >> 14 & 0x1f;
	pDst->mcs_15 = tmp82__ >> 19 & 0xf;
	pDst->eht_dup_6ghz = tmp82__ >> 23 & 0x1;
	pDst->op_sta_rx_ndp_wider_bw_20mhz = tmp82__ >> 24 & 0x1;
	pDst->non_ofdma_ul_mu_mimo_le_80mhz = tmp82__ >> 25 & 0x1;
	pDst->non_ofdma_ul_mu_mimo_160mhz = tmp82__ >> 26 & 0x1;
	pDst->non_ofdma_ul_mu_mimo_320mhz = tmp82__ >> 27 & 0x1;
	pDst->mu_bformer_le_80mhz = tmp82__ >> 28 & 0x1;
	pDst->mu_bformer_160mhz = tmp82__ >> 29 & 0x1;
	pDst->mu_bformer_320mhz = tmp82__ >> 30 & 0x1;
	pDst->tb_sounding_feedback_rl = tmp82__ >> 31 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->rx_1k_qam_in_wider_bw_dl_ofdma = tmp83__ >> 0 & 0x1;
	pDst->rx_4k_qam_in_wider_bw_dl_ofdma = tmp83__ >> 1 & 0x1;
	pDst->limited_cap_support_20mhz = tmp83__ >> 2 & 0x1;
	pDst->triggered_mu_bf_full_bw_fb_and_dl_mumimo = tmp83__ >> 3 & 0x1;
	pDst->mru_support_20mhz = tmp83__ >> 4 & 0x1;
	pDst->reserved3 = tmp83__ >> 5 & 0x7;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->bw_20_rx_max_nss_for_mcs_0_to_7 = tmp84__ >> 0 & 0xf;
	pDst->bw_20_tx_max_nss_for_mcs_0_to_7 = tmp84__ >> 4 & 0xf;
	pDst->bw_20_rx_max_nss_for_mcs_8_and_9 = tmp84__ >> 8 & 0xf;
	pDst->bw_20_tx_max_nss_for_mcs_8_and_9 = tmp84__ >> 12 & 0xf;
	pDst->bw_20_rx_max_nss_for_mcs_10_and_11 = tmp84__ >> 16 & 0xf;
	pDst->bw_20_tx_max_nss_for_mcs_10_and_11 = tmp84__ >> 20 & 0xf;
	pDst->bw_20_rx_max_nss_for_mcs_12_and_13 = tmp84__ >> 24 & 0xf;
	pDst->bw_20_tx_max_nss_for_mcs_12_and_13 = tmp84__ >> 28 & 0xf;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->bw_le_80_rx_max_nss_for_mcs_0_to_9 = tmp85__ >> 0 & 0xf;
	pDst->bw_le_80_tx_max_nss_for_mcs_0_to_9 = tmp85__ >> 4 & 0xf;
	pDst->bw_le_80_rx_max_nss_for_mcs_10_and_11 = tmp85__ >> 8 & 0xf;
	pDst->bw_le_80_tx_max_nss_for_mcs_10_and_11 = tmp85__ >> 12 & 0xf;
	pDst->bw_le_80_rx_max_nss_for_mcs_12_and_13 = tmp85__ >> 16 & 0xf;
	pDst->bw_le_80_tx_max_nss_for_mcs_12_and_13 = tmp85__ >> 20 & 0xf;
	pDst->bw_160_rx_max_nss_for_mcs_0_to_9 = tmp85__ >> 24 & 0xf;
	pDst->bw_160_tx_max_nss_for_mcs_0_to_9 = tmp85__ >> 28 & 0xf;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->bw_160_rx_max_nss_for_mcs_10_and_11 = tmp86__ >> 0 & 0xf;
	pDst->bw_160_tx_max_nss_for_mcs_10_and_11 = tmp86__ >> 4 & 0xf;
	pDst->bw_160_rx_max_nss_for_mcs_12_and_13 = tmp86__ >> 8 & 0xf;
	pDst->bw_160_tx_max_nss_for_mcs_12_and_13 = tmp86__ >> 12 & 0xf;
	pDst->bw_320_rx_max_nss_for_mcs_0_to_9 = tmp86__ >> 16 & 0xf;
	pDst->bw_320_tx_max_nss_for_mcs_0_to_9 = tmp86__ >> 20 & 0xf;
	pDst->bw_320_rx_max_nss_for_mcs_10_and_11 = tmp86__ >> 24 & 0xf;
	pDst->bw_320_tx_max_nss_for_mcs_10_and_11 = tmp86__ >> 28 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->bw_320_rx_max_nss_for_mcs_12_and_13 = tmp87__ >> 0 & 0xf;
	pDst->bw_320_tx_max_nss_for_mcs_12_and_13 = tmp87__ >> 4 & 0xf;
				 ...
				 bool append_ie)
{
	...
	pBuf += 1;
	ielen -= 1;
	pDst->eht_op_information_present = tmp88__ >> 0 & 0x1;
	pDst->disabled_sub_chan_bitmap_present = tmp88__ >> 1 & 0x1;
	pDst->eht_default_pe_duration = tmp88__ >> 2 & 0x1;
	pDst->group_addr_bu_indication_limit = tmp88__ >> 3 & 0x1;
	pDst->group_addr_bu_indication_exponent = tmp88__ >> 4 & 0x3;
	pDst->reserved = tmp88__ >> 6 & 0x3;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->basic_rx_max_nss_for_mcs_0_to_7 = tmp89__ >> 0 & 0xf;
	pDst->basic_tx_max_nss_for_mcs_0_to_7 = tmp89__ >> 4 & 0xf;
	pDst->basic_rx_max_nss_for_mcs_8_and_9 = tmp89__ >> 8 & 0xf;
	pDst->basic_tx_max_nss_for_mcs_8_and_9 = tmp89__ >> 12 & 0xf;
	pDst->basic_rx_max_nss_for_mcs_10_and_11 = tmp89__ >> 16 & 0xf;
	pDst->basic_tx_max_nss_for_mcs_10_and_11 = tmp89__ >> 20 & 0xf;
	pDst->basic_rx_max_nss_for_mcs_12_and_13 = tmp89__ >> 24 & 0xf;
	pDst->basic_tx_max_nss_for_mcs_12_and_13 = tmp89__ >> 28 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->channel_width = tmp90__ >> 0 & 0x7;
	pDst->reserved_1 = tmp90__ >> 3 & 0x1f;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
					  ...
					  bool append_ie)
{
	...
	pBuf += 2;
	ielen -= 2;
	pDst->public_key_identifiers_cnt = tmp91__ >> 0 & 0x7;
	pDst->realm_identifiers_cnt = tmp91__ >> 3 & 0x7;
	pDst->is_ip_config_supported = tmp91__ >> 6 & 0x1;
	pDst->is_cache_id_present = tmp91__ >> 7 & 0x1;
	pDst->is_hessid_present = tmp91__ >> 8 & 0x1;
	pDst->is_fils_sk_auth_supported = tmp91__ >> 9 & 0x1;
	pDst->is_fils_sk_auth_pfs_supported = tmp91__ >> 10 & 0x1;
	pDst->is_pk_auth_supported = tmp91__ >> 11 & 0x1;
	pDst->reserved = tmp91__ >> 12 & 0xf;
					   ...
					   bool append_ie)
{
	...
	pDst->min_mpdu_start_spacing = tmp92__ >> 0 & 0x7;
	pDst->max_ampdu_len_exp = tmp92__ >> 3 & 0x7;
	pDst->max_mpdu_len = tmp92__ >> 6 & 0x7;
	pDst->sm_pow_save = tmp92__ >> 9 & 0x3;
	pDst->rd_responder = tmp92__ >> 11 & 0x1;
	pDst->rx_ant_pattern_consistency = tmp92__ >> 12 & 0x1;
	pDst->tx_ant_pattern_consistency = tmp92__ >> 13 & 0x1;
	pDst->reserved = tmp92__ >> 14 & 0x3;
	(void)pCtx;
	return status;
				 ...
				 bool append_ie)
{
	...
	pBuf += 4;
	ielen -= 4;
	pDst->htc_he = tmp93__ >> 0 & 0x1;
	pDst->twt_request = tmp93__ >> 1 & 0x1;
	pDst->twt_responder = tmp93__ >> 2 & 0x1;
	pDst->fragmentation = tmp93__ >> 3 & 0x3;
	pDst->max_num_frag_msdu_amsdu_exp = tmp93__ >> 5 & 0x7;
	pDst->min_frag_size = tmp93__ >> 8 & 0x3;
	pDst->trigger_frm_mac_pad = tmp93__ >> 10 & 0x3;
	pDst->multi_tid_aggr_rx_supp = tmp93__ >> 12 & 0x7;
	pDst->he_link_adaptation = tmp93__ >> 15 & 0x3;
	pDst->all_ack = tmp93__ >> 17 & 0x1;
	pDst->trigd_rsp_sched = tmp93__ >> 18 & 0x1;
	pDst->a_bsr = tmp93__ >> 19 & 0x1;
	pDst->broadcast_twt = tmp93__ >> 20 & 0x1;
	pDst->ba_32bit_bitmap = tmp93__ >> 21 & 0x1;
	pDst->mu_cascade = tmp93__ >> 22 & 0x1;
	pDst->ack_enabled_multitid = tmp93__ >> 23 & 0x1;
	pDst->reserved = tmp93__ >> 24 & 0x1;
	pDst->omi_a_ctrl = tmp93__ >> 25 & 0x1;
	pDst->ofdma_ra = tmp93__ >> 26 & 0x1;
	pDst->max_ampdu_len_exp_ext = tmp93__ >> 27 & 0x3;
	pDst->amsdu_frag = tmp93__ >> 29 & 0x1;
	pDst->flex_twt_sched = tmp93__ >> 30 & 0x1;
	pDst->rx_ctrl_frame = tmp93__ >> 31 & 0x1;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 2;
	ielen -= 2;
	pDst->bsrp_ampdu_aggr = tmp94__ >> 0 & 0x1;
	pDst->qtp = tmp94__ >> 1 & 0x1;
	pDst->a_bqr = tmp94__ >> 2 & 0x1;
	pDst->spatial_reuse_param_rspder = tmp94__ >> 3 & 0x1;
	pDst->ndp_feedback_supp = tmp94__ >> 4 & 0x1;
	pDst->ops_supp = tmp94__ >> 5 & 0x1;
	pDst->amsdu_in_ampdu = tmp94__ >> 6 & 0x1;
	pDst->multi_tid_aggr_tx_supp = tmp94__ >> 7 & 0x7;
	pDst->he_sub_ch_sel_tx_supp = tmp94__ >> 10 & 0x1;
	pDst->ul_2x996_tone_ru_supp = tmp94__ >> 11 & 0x1;
	pDst->om_ctrl_ul_mu_data_dis_rx = tmp94__ >> 12 & 0x1;
	pDst->he_dynamic_smps = tmp94__ >> 13 & 0x1;
	pDst->punctured_sounding_supp = tmp94__ >> 14 & 0x1;
	pDst->ht_vht_trg_frm_rx_supp = tmp94__ >> 15 & 0x1;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->reserved2 = tmp95__ >> 0 & 0x1;
	pDst->chan_width_0 = tmp95__ >> 1 & 0x1;
	pDst->chan_width_1 = tmp95__ >> 2 & 0x1;
	pDst->chan_width_2 = tmp95__ >> 3 & 0x1;
	pDst->chan_width_3 = tmp95__ >> 4 & 0x1;
	pDst->chan_width_4 = tmp95__ >> 5 & 0x1;
	pDst->chan_width_5 = tmp95__ >> 6 & 0x1;
	pDst->chan_width_6 = tmp95__ >> 7 & 0x1;
	pDst->rx_pream_puncturing = tmp95__ >> 8 & 0xf;
	pDst->device_class = tmp95__ >> 12 & 0x1;
	pDst->ldpc_coding = tmp95__ >> 13 & 0x1;
	pDst->he_1x_ltf_800_gi_ppdu = tmp95__ >> 14 & 0x1;
	pDst->midamble_tx_rx_max_nsts = tmp95__ >> 15 & 0x3;
	pDst->he_4x_ltf_3200_gi_ndp = tmp95__ >> 17 & 0x1;
	pDst->tb_ppdu_tx_stbc_lt_80mhz = tmp95__ >> 18 & 0x1;
	pDst->rx_stbc_lt_80mhz = tmp95__ >> 19 & 0x1;
	pDst->doppler = tmp95__ >> 20 & 0x3;
	pDst->ul_mu = tmp95__ >> 22 & 0x3;
	pDst->dcm_enc_tx = tmp95__ >> 24 & 0x7;
	pDst->dcm_enc_rx = tmp95__ >> 27 & 0x7;
	pDst->ul_he_mu = tmp95__ >> 30 & 0x1;
	pDst->su_beamformer = tmp95__ >> 31 & 0x1;
	if (unlikely(ielen < 4)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 4;
	ielen -= 4;
	pDst->su_beamformee = tmp96__ >> 0 & 0x1;
	pDst->mu_beamformer = tmp96__ >> 1 & 0x1;
	pDst->bfee_sts_lt_80 = tmp96__ >> 2 & 0x7;
	pDst->bfee_sts_gt_80 = tmp96__ >> 5 & 0x7;
	pDst->num_sounding_lt_80 = tmp96__ >> 8 & 0x7;
	pDst->num_sounding_gt_80 = tmp96__ >> 11 & 0x7;
	pDst->su_feedback_tone16 = tmp96__ >> 14 & 0x1;
	pDst->mu_feedback_tone16 = tmp96__ >> 15 & 0x1;
	pDst->codebook_su = tmp96__ >> 16 & 0x1;
	pDst->codebook_mu = tmp96__ >> 17 & 0x1;
	pDst->beamforming_feedback = tmp96__ >> 18 & 0x7;
	pDst->he_er_su_ppdu = tmp96__ >> 21 & 0x1;
	pDst->dl_mu_mimo_part_bw = tmp96__ >> 22 & 0x1;
	pDst->ppet_present = tmp96__ >> 23 & 0x1;
	pDst->srp = tmp96__ >> 24 & 0x1;
	pDst->power_boost = tmp96__ >> 25 & 0x1;
	pDst->he_ltf_800_gi_4x = tmp96__ >> 26 & 0x1;
	pDst->max_nc = tmp96__ >> 27 & 0x7;
	pDst->tb_ppdu_tx_stbc_gt_80mhz = tmp96__ >> 30 & 0x1;
	pDst->rx_stbc_gt_80mhz = tmp96__ >> 31 & 0x1;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 2;
	ielen -= 2;
	pDst->er_he_ltf_800_gi_4x = tmp97__ >> 0 & 0x1;
	pDst->he_ppdu_20_in_40Mhz_2G = tmp97__ >> 1 & 0x1;
	pDst->he_ppdu_20_in_160_80p80Mhz = tmp97__ >> 2 & 0x1;
	pDst->he_ppdu_80_in_160_80p80Mhz = tmp97__ >> 3 & 0x1;
	pDst->er_1x_he_ltf_gi = tmp97__ >> 4 & 0x1;
	pDst->midamble_tx_rx_1x_he_ltf = tmp97__ >> 5 & 0x1;
	pDst->dcm_max_bw = tmp97__ >> 6 & 0x3;
	pDst->longer_than_16_he_sigb_ofdm_sym = tmp97__ >> 8 & 0x1;
	pDst->non_trig_cqi_feedback = tmp97__ >> 9 & 0x1;
	pDst->tx_1024_qam_lt_242_tone_ru = tmp97__ >> 10 & 0x1;
	pDst->rx_1024_qam_lt_242_tone_ru = tmp97__ >> 11 & 0x1;
	pDst->rx_full_bw_su_he_mu_compress_sigb = tmp97__ >> 12 & 0x1;
	pDst->rx_full_bw_su_he_mu_non_cmpr_sigb = tmp97__ >> 13 & 0x1;
	pDst->reserved3 = tmp97__ >> 14 & 0x3;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
				 ...
				 bool append_ie)
{
	...
	pBuf += 2;
	ielen -= 2;
	pDst->default_pe = tmp98__ >> 0 & 0x7;
	pDst->twt_required = tmp98__ >> 3 & 0x1;
	pDst->txop_rts_threshold = tmp98__ >> 4 & 0x3ff;
	pDst->vht_oper_present = tmp98__ >> 14 & 0x1;
	pDst->co_located_bss = tmp98__ >> 15 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->er_su_disable = tmp99__ >> 0 & 0x1;
	pDst->oper_info_6g_present = tmp99__ >> 1 & 0x1;
	pDst->reserved2 = tmp99__ >> 2 & 0x3f;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->bss_color = tmp100__ >> 0 & 0x3f;
	pDst->partial_bss_col = tmp100__ >> 6 & 0x1;
	pDst->bss_col_disabled = tmp100__ >> 7 & 0x1;
	if (unlikely(ielen < 2)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	case 1:
		...
		pBuf += 1;
		ielen -= 1;
		pDst->oper_info_6g.info.ch_width = tmp101__ >> 0 & 0x3;
		pDst->oper_info_6g.info.dup_bcon = tmp101__ >> 2 & 0x1;
		pDst->oper_info_6g.info.reg_info = tmp101__ >> 3 & 0x7;
		pDst->oper_info_6g.info.reserved = tmp101__ >> 6 & 0x3;
		if (unlikely(ielen < 1)) {
			pDst->present = 0;
			return DOT11F_INCOMPLETE_IE;
		}
					 ...
					 bool append_ie)
{
	...
	pBuf += 1;
	ielen -= 1;
	pDst->dgaf_dis = tmp102__ >> 0 & 0x1;
	pDst->hs_id_present = tmp102__ >> 1 & 0x3;
	pDst->reserved = tmp102__ >> 3 & 0x1;
	pDst->release_num = tmp102__ >> 4 & 0xf;
	if (!ielen) {
		return 0U;
	} else {
						 ...
						 bool append_ie)
{
	...
	tmp103__ = *pBuf;
	pDst->info_request = tmp103__ >> 0 & 0x1;
	pDst->forty_mhz_intolerant = tmp103__ >> 1 & 0x1;
	pDst->twenty_mhz_bsswidth_req = tmp103__ >> 2 & 0x1;
	pDst->obss_scan_exemption_req = tmp103__ >> 3 & 0x1;
	pDst->obss_scan_exemption_grant = tmp103__ >> 4 & 0x1;
	pDst->unused = tmp103__ >> 5 & 0x7;
	(void)pCtx;
	return status;
					    ...
					    bool append_ie)
{
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbe_aifsn = tmp104__ >> 0 & 0xf;
	pDst->acbe_acm = tmp104__ >> 4 & 0x1;
	pDst->acbe_aci = tmp104__ >> 5 & 0x3;
	pDst->unused1 = tmp104__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbe_acwmin = tmp105__ >> 0 & 0xf;
	pDst->acbe_acwmax = tmp105__ >> 4 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbk_aifsn = tmp106__ >> 0 & 0xf;
	pDst->acbk_acm = tmp106__ >> 4 & 0x1;
	pDst->acbk_aci = tmp106__ >> 5 & 0x3;
	pDst->unused2 = tmp106__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acbk_acwmin = tmp107__ >> 0 & 0xf;
	pDst->acbk_acwmax = tmp107__ >> 4 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvi_aifsn = tmp108__ >> 0 & 0xf;
	pDst->acvi_acm = tmp108__ >> 4 & 0x1;
	pDst->acvi_aci = tmp108__ >> 5 & 0x3;
	pDst->unused3 = tmp108__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvi_acwmin = tmp109__ >> 0 & 0xf;
	pDst->acvi_acwmax = tmp109__ >> 4 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvo_aifsn = tmp110__ >> 0 & 0xf;
	pDst->acvo_acm = tmp110__ >> 4 & 0x1;
	pDst->acvo_aci = tmp110__ >> 5 & 0x3;
	pDst->unused4 = tmp110__ >> 7 & 0x1;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	pBuf += 1;
	ielen -= 1;
	pDst->acvo_acwmin = tmp111__ >> 0 & 0xf;
	pDst->acvo_acwmax = tmp111__ >> 4 & 0xf;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
						  ...
						  bool append_ie)
{
	...
	pBuf += 2;
	ielen -= 2;
	pDst->tbtt_type = tmp112__ >> 0 & 0x3;
	pDst->filtered_neighbor_ap = tmp112__ >> 2 & 0x1;
	pDst->reserved = tmp112__ >> 3 & 0x1;
	pDst->tbtt_info_count = tmp112__ >> 4 & 0xf;
	pDst->tbtt_info_len = tmp112__ >> 8 & 0xff;
	if (unlikely(ielen < 1)) {
		pDst->present = 0;
		return DOT11F_INCOMPLETE_IE;
	}
	...
	case 16:
		...
		pBuf += 2;
		ielen -= 2;
		pDst->tbtt_info.tbtt_info_16.link_id = tmp113__ >> 0 & 0xf;
		pDst->tbtt_info.tbtt_info_16.bss_param_change_cnt = tmp113__ >> 4 & 0xff;
		pDst->tbtt_info.tbtt_info_16.all_updates_included = tmp113__ >> 12 & 0x1;
		pDst->tbtt_info.tbtt_info_16.reserved = tmp113__ >> 13 & 0x7;
		break;
	}
	(void)pCtx;
	return status;
					 ...
					 bool append_ie)
{
	...
	pBuf += 1;
	ielen -= 1;
	pDst->psr_disallow = tmp114__ >> 0 & 0x1;
	pDst->non_srg_pd_sr_disallow = tmp114__ >> 1 & 0x1;
	pDst->non_srg_offset_present = tmp114__ >> 2 & 0x1;
	pDst->srg_info_present = tmp114__ >> 3 & 0x1;
	pDst->sr_value15_allow = tmp114__ >> 4 & 0x1;
	pDst->reserved = tmp114__ >> 5 & 0x7;
...
static const tIEDefn IES_Beacon[] = {
	...
	8, 4, 20, SigIetransmit_power_env, {0, 0, 0, 0, 0},
	0, DOT11F_EID_TRANSMIT_POWER_ENV, 0, 0, },
	{ offsetof(tDot11fBeacon, ChannelSwitchWrapper),
	offsetof(tDot11fIEChannelSwitchWrapper, present), 0,
	"ChannelSwitchWrapper", 0, 2, 27, SigIeChannelSwitchWrapper,
...
static const tIEDefn IES_BeaconIEs[] = {
	...
	"transmit_power_env", 8, 4, 20, SigIetransmit_power_env, {0, 0, 0, 0, 0},
	0, DOT11F_EID_TRANSMIT_POWER_ENV, 0, 0, },
...
uint32_t dot11f_get_packed_ie_transmit_power_env(tpAniSirGlobal pCtx,
	tDot11fIEtransmit_power_env *pIe, uint32_t *pnNeeded)
{
	uint32_t status = DOT11F_PARSE_SUCCESS;
	(void)pCtx;
	while (pIe->present) {
		*pnNeeded += 1;
		*pnNeeded += pIe->num_tx_power;
		if (pIe->max_tx_pwr_interpret) {
			switch (pIe->max_tx_pwr_interpret) {
			case 0:
				*pnNeeded += 1;
				break;
			case 1:
	...
	return status;
} /* End dot11f_get_packed_ie_transmit_power_env. */
			...
			for (i = 0U; i < countOffset; ++i) {
				*pnNeeded += 2U + pIe->noui;
				...
				case SigIetransmit_power_env:
					...
					status |=
						dot11f_get_packed_ie_transmit_power_env(
				      ...
				      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 70;
		...
		++pBuf; ++(*pnConsumed);
		tmp128__ = 0U;
		tmp128__ |= (pSrc->LinkMeasurement << 0);
		tmp128__ |= (pSrc->NeighborRpt << 1);
		tmp128__ |= (pSrc->parallel << 2);
		tmp128__ |= (pSrc->repeated << 3);
		tmp128__ |= (pSrc->BeaconPassive << 4);
		tmp128__ |= (pSrc->BeaconActive << 5);
		tmp128__ |= (pSrc->BeaconTable << 6);
		tmp128__ |= (pSrc->BeaconRepCond << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp129__ = 0U;
		tmp129__ |= (pSrc->FrameMeasurement << 0);
		tmp129__ |= (pSrc->ChannelLoad << 1);
		tmp129__ |= (pSrc->NoiseHistogram << 2);
		tmp129__ |= (pSrc->statistics << 3);
		tmp129__ |= (pSrc->LCIMeasurement << 4);
		tmp129__ |= (pSrc->LCIAzimuth << 5);
		tmp129__ |= (pSrc->TCMCapability << 6);
		tmp129__ |= (pSrc->triggeredTCM << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp130__ = 0U;
		tmp130__ |= (pSrc->APChanReport << 0);
		tmp130__ |= (pSrc->RRMMIBEnabled << 1);
		tmp130__ |= (pSrc->operatingChanMax << 2);
		tmp130__ |= (pSrc->nonOperatinChanMax << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp131__ = 0U;
		tmp131__ |= (pSrc->MeasurementPilot << 0);
		tmp131__ |= (pSrc->MeasurementPilotEnabled << 3);
		tmp131__ |= (pSrc->NeighborTSFOffset << 4);
		tmp131__ |= (pSrc->RCPIMeasurement << 5);
		tmp131__ |= (pSrc->RSNIMeasurement << 6);
		tmp131__ |= (pSrc->BssAvgAccessDelay << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp132__ = 0U;
		tmp132__ |= (pSrc->BSSAvailAdmission << 0);
		tmp132__ |= (pSrc->AntennaInformation << 1);
		tmp132__ |= (pSrc->fine_time_meas_rpt << 2);
		tmp132__ |= (pSrc->lci_capability << 3);
				 ...
				 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 15;
		...
		++pBuf; ++(*pnConsumed);
		tmp133__ = 0U;
		tmp133__ |= (pSrc->aggregation << 0);
		tmp133__ |= (pSrc->tsid << 1);
		tmp133__ |= (pSrc->direction << 5);
		tmp133__ |= (pSrc->reserved << 7);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			      ...
			      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 13;
		...
		++pBuf; ++(*pnConsumed);
		tmp134__ = 0U;
		tmp134__ |= (pSrc->traffic_type << 0);
		tmp134__ |= (pSrc->tsid << 1);
		tmp134__ |= (pSrc->direction << 5);
		tmp134__ |= (pSrc->access_policy << 7);
		tmp134__ |= (pSrc->aggregation << 9);
		tmp134__ |= (pSrc->psb << 10);
		tmp134__ |= (pSrc->user_priority << 11);
		tmp134__ |= (pSrc->tsinfo_ack_pol << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp135__ = 0U;
		tmp135__ |= (pSrc->schedule << 0);
		tmp135__ |= (pSrc->unused << 1);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp136__ = 0U;
		tmp136__ |= (pSrc->size << 0);
		tmp136__ |= (pSrc->fixed << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				...
				uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 191;
		...
		++pBuf; ++(*pnConsumed);
		tmp137__ = 0U;
		tmp137__ |= (pSrc->maxMPDULen << 0);
		tmp137__ |= (pSrc->supportedChannelWidthSet << 2);
		tmp137__ |= (pSrc->ldpcCodingCap << 4);
		tmp137__ |= (pSrc->shortGI80MHz << 5);
		tmp137__ |= (pSrc->shortGI160and80plus80MHz << 6);
		tmp137__ |= (pSrc->txSTBC << 7);
		tmp137__ |= (pSrc->rxSTBC << 8);
		tmp137__ |= (pSrc->suBeamFormerCap << 11);
		tmp137__ |= (pSrc->suBeamformeeCap << 12);
		tmp137__ |= (pSrc->csnofBeamformerAntSup << 13);
		tmp137__ |= (pSrc->numSoundingDim << 16);
		tmp137__ |= (pSrc->muBeamformerCap << 19);
		tmp137__ |= (pSrc->muBeamformeeCap << 20);
		tmp137__ |= (pSrc->vhtTXOPPS << 21);
		tmp137__ |= (pSrc->htcVHTCap << 22);
		tmp137__ |= (pSrc->maxAMPDULenExp << 23);
		tmp137__ |= (pSrc->vhtLinkAdaptCap << 26);
		tmp137__ |= (pSrc->rxAntPattern << 28);
		tmp137__ |= (pSrc->txAntPattern << 29);
		tmp137__ |= (pSrc->extended_nss_bw_supp << 30);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		pBuf += 2;
		tmp138__ = 0U;
		tmp138__ |= (pSrc->rxHighSupDataRate << 0);
		tmp138__ |= (pSrc->max_nsts_total << 13);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		pBuf += 2;
		tmp139__ = 0U;
		tmp139__ |= (pSrc->txSupDataRate << 0);
		tmp139__ |= (pSrc->vht_extended_nss_bw_cap << 13);
		tmp139__ |= (pSrc->reserved << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				 ...
				 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp141__ = 0U;
		tmp141__ |= (pSrc->traffic_type << 0);
		tmp141__ |= (pSrc->tsid << 1);
		tmp141__ |= (pSrc->direction << 5);
		tmp141__ |= (pSrc->access_policy << 7);
		tmp141__ |= (pSrc->aggregation << 9);
		tmp141__ |= (pSrc->psb << 10);
		tmp141__ |= (pSrc->user_priority << 11);
		tmp141__ |= (pSrc->tsinfo_ack_pol << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp142__ = 0U;
		tmp142__ |= (pSrc->tsinfo_rsvd << 0);
		tmp142__ |= (pSrc->burst_size_defn << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp143__ = 0U;
		tmp143__ |= (pSrc->size << 0);
		tmp143__ |= (pSrc->fixed << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
							   ...
							   uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 2;
		...
		++pBuf; ++(*pnConsumed);
		tmp144__ = 0U;
		tmp144__ |= (pSrc->beacon_report_id << 0);
		tmp144__ |= (pSrc->fragment_id_number << 8);
		tmp144__ |= (pSrc->more_fragments << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
				     ...
				     uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pnConsumed += 6;
		pBuf += 6;
		tmp145__ = 0U;
		tmp145__ |= (pSrc->APReachability << 0);
		tmp145__ |= (pSrc->Security << 2);
		tmp145__ |= (pSrc->KeyScope << 3);
		tmp145__ |= (pSrc->SpecMgmtCap << 4);
		tmp145__ |= (pSrc->QosCap << 5);
		tmp145__ |= (pSrc->apsd << 6);
		tmp145__ |= (pSrc->rrm << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp146__ = 0U;
		tmp146__ |= (pSrc->DelayedBA << 0);
		tmp146__ |= (pSrc->ImmBA << 1);
		tmp146__ |= (pSrc->MobilityDomain << 2);
					   ...
					   uint32_t *pnConsumed)
{
	...
	uint8_t tmp149__;
	uint32_t status = DOT11F_PARSE_SUCCESS;
	status = dot11f_get_packed_ie_transmit_power_env(pCtx, pSrc, &nNeeded);
	if (!DOT11F_SUCCEEDED(status))
		return status;
	while (pSrc->present) {
		if (nNeeded > nBuf)
		...
		*pnConsumed += pSrc->num_tx_power;
		pBuf += pSrc->num_tx_power;
		if (pSrc->max_tx_pwr_interpret) {
			switch (pSrc->max_tx_pwr_interpret) {
			case 0:
				*pBuf = pSrc->ext_max_tx_power.ext_max_tx_power_local_eirp.max_tx_power_for_320;
				*pnConsumed += 1;
				/* fieldsEndFlag = 1 */
				break;
			case 1:
				tmp148__ = 0U;
				tmp148__ |= (pSrc->ext_max_tx_power.ext_max_tx_power_local_psd.ext_count << 0);
				tmp148__ |= (pSrc->ext_max_tx_power.ext_max_tx_power_local_psd.reserved << 4);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
				...
				*pnConsumed += 1;
				pBuf += 1;
				nBuf -=  1 ;
				DOT11F_MEMCPY(pCtx, pBuf, pSrc->ext_max_tx_power.ext_max_tx_power_local_psd.max_tx_psd_power, 8);
				*pnConsumed += 8;
				/* fieldsEndFlag = 1 */
				break;
			case 2:
				*pBuf = pSrc->ext_max_tx_power.ext_max_tx_power_reg_eirp.max_tx_power_for_320;
				*pnConsumed += 1;
				/* fieldsEndFlag = 1 */
				break;
			case 3:
				tmp149__ = 0U;
				tmp149__ |= (pSrc->ext_max_tx_power.ext_max_tx_power_reg_psd.ext_count << 0);
				tmp149__ |= (pSrc->ext_max_tx_power.ext_max_tx_power_reg_psd.reserved << 4);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
				...
				*pnConsumed += 1;
				pBuf += 1;
				nBuf -=  1 ;
				DOT11F_MEMCPY(pCtx, pBuf, pSrc->ext_max_tx_power.ext_max_tx_power_reg_psd.max_tx_psd_power, 8);
				*pnConsumed += 8;
				/* fieldsEndFlag = 1 */
				break;
			}
				...
				uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 108;
		...
		tmp195__ |= (pSrc->eht_trs_support << 9);
		tmp195__ |= (pSrc->txop_return_support_txop_share_m2 << 10);
		tmp195__ |= (pSrc->two_bqrs_support << 11);
		tmp195__ |= (pSrc->eht_link_adaptation_support << 12);
		tmp195__ |= (pSrc->reserved << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		tmp198__ |= (pSrc->limited_cap_support_20mhz << 2);
		tmp198__ |= (pSrc->triggered_mu_bf_full_bw_fb_and_dl_mumimo << 3);
		tmp198__ |= (pSrc->mru_support_20mhz << 4);
		tmp198__ |= (pSrc->reserved3 << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
			       ...
			       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 106;
		...
		tmp203__ |= (pSrc->eht_default_pe_duration << 2);
		tmp203__ |= (pSrc->group_addr_bu_indication_limit << 3);
		tmp203__ |= (pSrc->group_addr_bu_indication_exponent << 4);
		tmp203__ |= (pSrc->reserved << 6);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp204__ = 0U;
		tmp204__ |= (pSrc->basic_rx_max_nss_for_mcs_0_to_7 << 0);
		tmp204__ |= (pSrc->basic_tx_max_nss_for_mcs_0_to_7 << 4);
		tmp204__ |= (pSrc->basic_rx_max_nss_for_mcs_8_and_9 << 8);
		tmp204__ |= (pSrc->basic_tx_max_nss_for_mcs_8_and_9 << 12);
		tmp204__ |= (pSrc->basic_rx_max_nss_for_mcs_10_and_11 << 16);
		tmp204__ |= (pSrc->basic_tx_max_nss_for_mcs_10_and_11 << 20);
		tmp204__ |= (pSrc->basic_rx_max_nss_for_mcs_12_and_13 << 24);
		tmp204__ |= (pSrc->basic_tx_max_nss_for_mcs_12_and_13 << 28);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp205__ = 0U;
		tmp205__ |= (pSrc->channel_width << 0);
		tmp205__ |= (pSrc->reserved_1 << 3);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
					...
					uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 240;
		...
		++pBuf; ++(*pnConsumed);
		tmp206__ = 0U;
		tmp206__ |= (pSrc->public_key_identifiers_cnt << 0);
		tmp206__ |= (pSrc->realm_identifiers_cnt << 3);
		tmp206__ |= (pSrc->is_ip_config_supported << 6);
		tmp206__ |= (pSrc->is_cache_id_present << 7);
		tmp206__ |= (pSrc->is_hessid_present << 8);
		tmp206__ |= (pSrc->is_fils_sk_auth_supported << 9);
		tmp206__ |= (pSrc->is_fils_sk_auth_pfs_supported << 10);
		tmp206__ |= (pSrc->is_pk_auth_supported << 11);
		tmp206__ |= (pSrc->reserved << 12);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
					 ...
					 uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 59;
		++pBuf; ++(*pnConsumed);
		tmp207__ = 0U;
		tmp207__ |= (pSrc->min_mpdu_start_spacing << 0);
		tmp207__ |= (pSrc->max_ampdu_len_exp << 3);
		tmp207__ |= (pSrc->max_mpdu_len << 6);
		tmp207__ |= (pSrc->sm_pow_save << 9);
		tmp207__ |= (pSrc->rd_responder << 11);
		tmp207__ |= (pSrc->rx_ant_pattern_consistency << 12);
		tmp207__ |= (pSrc->tx_ant_pattern_consistency << 13);
		tmp207__ |= (pSrc->reserved << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			       ...
			       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 35;
		++pBuf; ++(*pnConsumed);
		tmp208__ = 0U;
		tmp208__ |= (pSrc->htc_he << 0);
		tmp208__ |= (pSrc->twt_request << 1);
		tmp208__ |= (pSrc->twt_responder << 2);
		tmp208__ |= (pSrc->fragmentation << 3);
		tmp208__ |= (pSrc->max_num_frag_msdu_amsdu_exp << 5);
		tmp208__ |= (pSrc->min_frag_size << 8);
		tmp208__ |= (pSrc->trigger_frm_mac_pad << 10);
		tmp208__ |= (pSrc->multi_tid_aggr_rx_supp << 12);
		tmp208__ |= (pSrc->he_link_adaptation << 15);
		tmp208__ |= (pSrc->all_ack << 17);
		tmp208__ |= (pSrc->trigd_rsp_sched << 18);
		tmp208__ |= (pSrc->a_bsr << 19);
		tmp208__ |= (pSrc->broadcast_twt << 20);
		tmp208__ |= (pSrc->ba_32bit_bitmap << 21);
		tmp208__ |= (pSrc->mu_cascade << 22);
		tmp208__ |= (pSrc->ack_enabled_multitid << 23);
		tmp208__ |= (pSrc->reserved << 24);
		tmp208__ |= (pSrc->omi_a_ctrl << 25);
		tmp208__ |= (pSrc->ofdma_ra << 26);
		tmp208__ |= (pSrc->max_ampdu_len_exp_ext << 27);
		tmp208__ |= (pSrc->amsdu_frag << 29);
		tmp208__ |= (pSrc->flex_twt_sched << 30);
		tmp208__ |= (pSrc->rx_ctrl_frame << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp209__ = 0U;
		tmp209__ |= (pSrc->bsrp_ampdu_aggr << 0);
		tmp209__ |= (pSrc->qtp << 1);
		tmp209__ |= (pSrc->a_bqr << 2);
		tmp209__ |= (pSrc->spatial_reuse_param_rspder << 3);
		tmp209__ |= (pSrc->ndp_feedback_supp << 4);
		tmp209__ |= (pSrc->ops_supp << 5);
		tmp209__ |= (pSrc->amsdu_in_ampdu << 6);
		tmp209__ |= (pSrc->multi_tid_aggr_tx_supp << 7);
		tmp209__ |= (pSrc->he_sub_ch_sel_tx_supp << 10);
		tmp209__ |= (pSrc->ul_2x996_tone_ru_supp << 11);
		tmp209__ |= (pSrc->om_ctrl_ul_mu_data_dis_rx << 12);
		tmp209__ |= (pSrc->he_dynamic_smps << 13);
		tmp209__ |= (pSrc->punctured_sounding_supp << 14);
		tmp209__ |= (pSrc->ht_vht_trg_frm_rx_supp << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp210__ = 0U;
		tmp210__ |= (pSrc->reserved2 << 0);
		tmp210__ |= (pSrc->chan_width_0 << 1);
		tmp210__ |= (pSrc->chan_width_1 << 2);
		tmp210__ |= (pSrc->chan_width_2 << 3);
		tmp210__ |= (pSrc->chan_width_3 << 4);
		tmp210__ |= (pSrc->chan_width_4 << 5);
		tmp210__ |= (pSrc->chan_width_5 << 6);
		tmp210__ |= (pSrc->chan_width_6 << 7);
		tmp210__ |= (pSrc->rx_pream_puncturing << 8);
		tmp210__ |= (pSrc->device_class << 12);
		tmp210__ |= (pSrc->ldpc_coding << 13);
		tmp210__ |= (pSrc->he_1x_ltf_800_gi_ppdu << 14);
		tmp210__ |= (pSrc->midamble_tx_rx_max_nsts << 15);
		tmp210__ |= (pSrc->he_4x_ltf_3200_gi_ndp << 17);
		tmp210__ |= (pSrc->tb_ppdu_tx_stbc_lt_80mhz << 18);
		tmp210__ |= (pSrc->rx_stbc_lt_80mhz << 19);
		tmp210__ |= (pSrc->doppler << 20);
		tmp210__ |= (pSrc->ul_mu << 22);
		tmp210__ |= (pSrc->dcm_enc_tx << 24);
		tmp210__ |= (pSrc->dcm_enc_rx << 27);
		tmp210__ |= (pSrc->ul_he_mu << 30);
		tmp210__ |= (pSrc->su_beamformer << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp211__ = 0U;
		tmp211__ |= (pSrc->su_beamformee << 0);
		tmp211__ |= (pSrc->mu_beamformer << 1);
		tmp211__ |= (pSrc->bfee_sts_lt_80 << 2);
		tmp211__ |= (pSrc->bfee_sts_gt_80 << 5);
		tmp211__ |= (pSrc->num_sounding_lt_80 << 8);
		tmp211__ |= (pSrc->num_sounding_gt_80 << 11);
		tmp211__ |= (pSrc->su_feedback_tone16 << 14);
		tmp211__ |= (pSrc->mu_feedback_tone16 << 15);
		tmp211__ |= (pSrc->codebook_su << 16);
		tmp211__ |= (pSrc->codebook_mu << 17);
		tmp211__ |= (pSrc->beamforming_feedback << 18);
		tmp211__ |= (pSrc->he_er_su_ppdu << 21);
		tmp211__ |= (pSrc->dl_mu_mimo_part_bw << 22);
		tmp211__ |= (pSrc->ppet_present << 23);
		tmp211__ |= (pSrc->srp << 24);
		tmp211__ |= (pSrc->power_boost << 25);
		tmp211__ |= (pSrc->he_ltf_800_gi_4x << 26);
		tmp211__ |= (pSrc->max_nc << 27);
		tmp211__ |= (pSrc->tb_ppdu_tx_stbc_gt_80mhz << 30);
		tmp211__ |= (pSrc->rx_stbc_gt_80mhz << 31);
		if (unlikely(nBuf < 4))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 4;
		...
		nBuf -=  4 ;
		tmp212__ = 0U;
		tmp212__ |= (pSrc->er_he_ltf_800_gi_4x << 0);
		tmp212__ |= (pSrc->he_ppdu_20_in_40Mhz_2G << 1);
		tmp212__ |= (pSrc->he_ppdu_20_in_160_80p80Mhz << 2);
		tmp212__ |= (pSrc->he_ppdu_80_in_160_80p80Mhz << 3);
		tmp212__ |= (pSrc->er_1x_he_ltf_gi << 4);
		tmp212__ |= (pSrc->midamble_tx_rx_1x_he_ltf << 5);
		tmp212__ |= (pSrc->dcm_max_bw << 6);
		tmp212__ |= (pSrc->longer_than_16_he_sigb_ofdm_sym << 8);
		tmp212__ |= (pSrc->non_trig_cqi_feedback << 9);
		tmp212__ |= (pSrc->tx_1024_qam_lt_242_tone_ru << 10);
		tmp212__ |= (pSrc->rx_1024_qam_lt_242_tone_ru << 11);
		tmp212__ |= (pSrc->rx_full_bw_su_he_mu_compress_sigb << 12);
		tmp212__ |= (pSrc->rx_full_bw_su_he_mu_non_cmpr_sigb << 13);
		tmp212__ |= (pSrc->reserved3 << 14);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
			      ...
			      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 36;
		++pBuf; ++(*pnConsumed);
		tmp213__ = 0U;
		tmp213__ |= (pSrc->default_pe << 0);
		tmp213__ |= (pSrc->twt_required << 3);
		tmp213__ |= (pSrc->txop_rts_threshold << 4);
		tmp213__ |= (pSrc->vht_oper_present << 14);
		tmp213__ |= (pSrc->co_located_bss << 15);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 2;
		...
		nBuf -=  2 ;
		tmp214__ = 0U;
		tmp214__ |= (pSrc->er_su_disable << 0);
		tmp214__ |= (pSrc->oper_info_6g_present << 1);
		tmp214__ |= (pSrc->reserved2 << 2);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp215__ = 0U;
		tmp215__ |= (pSrc->bss_color << 0);
		tmp215__ |= (pSrc->partial_bss_col << 6);
		tmp215__ |= (pSrc->bss_col_disabled << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		if (pSrc->oper_info_6g_present) {
			...
			case 1:
				...
				*pnConsumed += 1;
				pBuf += 1;
				tmp216__ = 0U;
				tmp216__ |= (pSrc->oper_info_6g.info.ch_width << 0);
				tmp216__ |= (pSrc->oper_info_6g.info.dup_bcon << 2);
				tmp216__ |= (pSrc->oper_info_6g.info.reg_info << 3);
				tmp216__ |= (pSrc->oper_info_6g.info.reserved << 6);
				if (unlikely(nBuf < 1))
					return DOT11F_INCOMPLETE_IE;
				      ...
				      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 0x10;
		++pBuf; ++(*pnConsumed);
		tmp217__ = 0U;
		tmp217__ |= (pSrc->dgaf_dis << 0);
		tmp217__ |= (pSrc->hs_id_present << 1);
		tmp217__ |= (pSrc->reserved << 3);
		tmp217__ |= (pSrc->release_num << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
					       ...
					       uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 72;
		...
		++pBuf; ++(*pnConsumed);
		tmp218__ = 0U;
		tmp218__ |= (pSrc->info_request << 0);
		tmp218__ |= (pSrc->forty_mhz_intolerant << 1);
		tmp218__ |= (pSrc->twenty_mhz_bsswidth_req << 2);
		tmp218__ |= (pSrc->obss_scan_exemption_req << 3);
		tmp218__ |= (pSrc->obss_scan_exemption_grant << 4);
		tmp218__ |= (pSrc->unused << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
					  ...
					  uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp219__ = 0U;
		tmp219__ |= (pSrc->acbe_aifsn << 0);
		tmp219__ |= (pSrc->acbe_acm << 4);
		tmp219__ |= (pSrc->acbe_aci << 5);
		tmp219__ |= (pSrc->unused1 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp220__ = 0U;
		tmp220__ |= (pSrc->acbe_acwmin << 0);
		tmp220__ |= (pSrc->acbe_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp221__ = 0U;
		tmp221__ |= (pSrc->acbk_aifsn << 0);
		tmp221__ |= (pSrc->acbk_acm << 4);
		tmp221__ |= (pSrc->acbk_aci << 5);
		tmp221__ |= (pSrc->unused2 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp222__ = 0U;
		tmp222__ |= (pSrc->acbk_acwmin << 0);
		tmp222__ |= (pSrc->acbk_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp223__ = 0U;
		tmp223__ |= (pSrc->acvi_aifsn << 0);
		tmp223__ |= (pSrc->acvi_acm << 4);
		tmp223__ |= (pSrc->acvi_aci << 5);
		tmp223__ |= (pSrc->unused3 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp224__ = 0U;
		tmp224__ |= (pSrc->acvi_acwmin << 0);
		tmp224__ |= (pSrc->acvi_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		pBuf += 1;
		tmp225__ = 0U;
		tmp225__ |= (pSrc->acvo_aifsn << 0);
		tmp225__ |= (pSrc->acvo_acm << 4);
		tmp225__ |= (pSrc->acvo_aci << 5);
		tmp225__ |= (pSrc->unused4 << 7);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
		...
		*pnConsumed += 1;
		...
		nBuf -=  1 ;
		tmp226__ = 0U;
		tmp226__ |= (pSrc->acvo_acwmin << 0);
		tmp226__ |= (pSrc->acvo_acwmax << 4);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
						...
						uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 201;
		...
		++pBuf; ++(*pnConsumed);
		tmp227__ = 0U;
		tmp227__ |= (pSrc->tbtt_type << 0);
		tmp227__ |= (pSrc->filtered_neighbor_ap << 2);
		tmp227__ |= (pSrc->reserved << 3);
		tmp227__ |= (pSrc->tbtt_info_count << 4);
		tmp227__ |= (pSrc->tbtt_info_len << 8);
		if (unlikely(nBuf < 2))
			return DOT11F_INCOMPLETE_IE;

		frameshtons(pCtx, pBuf, tmp227__, 0);
		*pnConsumed += 2;
		pBuf += 2;
		nBuf -=  2 ;
		*pBuf = pSrc->op_class;
		...
		*pnConsumed += 1;
		...
		case 16:
			...
			*pnConsumed += 1;
			pBuf += 1;
			tmp228__ = 0U;
			tmp228__ |= (pSrc->tbtt_info.tbtt_info_16.link_id << 0);
			tmp228__ |= (pSrc->tbtt_info.tbtt_info_16.bss_param_change_cnt << 4);
			tmp228__ |= (pSrc->tbtt_info.tbtt_info_16.all_updates_included << 12);
			tmp228__ |= (pSrc->tbtt_info.tbtt_info_16.reserved << 13);
			if (unlikely(nBuf < 2))
				return DOT11F_INCOMPLETE_IE;

			frameshtons(pCtx, pBuf, tmp228__, 0);
			*pnConsumed += 2;
			/* fieldsEndFlag  = 1 */
			nBuf -=  2 ;
			break;
				      ...
				      uint32_t *pnConsumed)
{
	...
	while (pSrc->present) {
		...
		*pBuf = 39;
		++pBuf; ++(*pnConsumed);
		tmp229__ = 0U;
		tmp229__ |= (pSrc->psr_disallow << 0);
		tmp229__ |= (pSrc->non_srg_pd_sr_disallow << 1);
		tmp229__ |= (pSrc->non_srg_offset_present << 2);
		tmp229__ |= (pSrc->srg_info_present << 3);
		tmp229__ |= (pSrc->sr_value15_allow << 4);
		tmp229__ |= (pSrc->reserved << 5);
		if (unlikely(nBuf < 1))
			return DOT11F_INCOMPLETE_IE;
